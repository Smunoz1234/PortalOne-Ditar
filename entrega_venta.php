...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		$("#CardCode").change(function(){
			...
			<?php if($edit==0&&$dt_LS==0&&$dt_OV==0&&$sw_error==0){?>
			$.ajax({
				type: "POST",
				url: "includes/procedimientos.php?type=7&objtype=15&cardcode="+carcode
			});
...
//			});
		...
		$("#Sucursal").change(function(){
			...
			$.ajax({
				type: "POST",
				url: "ajx_cbo_select.php?type=20&id="+Sucursal+"&serie="+Serie+"&tdoc=15",
				success: function(response){
			...
			if(Sucursal!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=3&type=1&name=OcrCode3&value="+Base64.encode(Sucursal)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=3&type=2&name=OcrCode3&value="+Base64.encode(Sucursal)+"&line=0&id=<?php echo $row['ID_EntregaVenta'];?>&evento=<?php echo $IdEvento;?>&actodos=1",
							success: function(response){
...
//			var frame=document.getElementById('DataGrid');			
//			if(carcode!="" && almacen!=""){
//				frame.src="detalle_entrega_venta.php?id=0&type=1&usr=<?php //echo $_SESSION['CodUser'];?>&cardcode="+carcode+"&whscode="+almacen;
//			}else{
//				frame.src="detalle_entrega_venta.php";
//			}
//			$('.ibox-content').toggleClass('sk-loading',false);
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="col-lg-8">
					...
					<div class="form-group">
						...
						<div class="col-lg-10">
							<textarea name="Comentarios" form="CrearEntregaVenta" rows="4" class="form-control" id="Comentarios" <?php if(($edit==1)&&($row['Cod_Estado']=='C')){echo "readonly";}?>><?php if($edit==1){echo $row['Comentarios'];}?></textarea>
						</div>
					</div>
					<div class="form-group">
				...
				<div class="form-group">
					<div class="col-lg-9">
						...
						<?php }elseif($edit==1&&$row['Cod_Estado']=="O"&&PermitirFuncion(404)){?>
							<button class="btn btn-warning" type="submit" form="CrearEntregaVenta" id="Actualizar"><i class="fa fa-refresh"></i> Actualizar Entrega de venta</button>
						<?php }?>
						<?php 
					...
					<div class="col-lg-3">
						<div class="btn-group pull-right">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="alkin dropdown-item" href="devolucion_venta.php?dt_ET=1&Cardcode=<?php echo base64_encode($row['CardCode']);?>&CCosto=<?php echo base64_encode($row['OcrCode2']);?>&Area=<?php echo base64_encode($row['OcrCode']);?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']);?>&Direccion=<?php echo base64_encode($row['DireccionDestino']);?>&Almacen=<?php echo base64_encode($row['WhsCode']);?>&Contacto=<?php echo base64_encode($row['CodigoContacto']);?>&Referencia=<?php echo base64_encode($row['NumAtCard']);?>&Empleado=<?php echo base64_encode($row['SlpCode']);?>&ET=<?php echo base64_encode($row['ID_EntregaVenta']);?>&Evento=<?php echo base64_encode($row['IdEvento']);?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']);?>">Devolución de venta</a></li>
                            </ul>
                        </div>
					</div>
					<?php }?>
