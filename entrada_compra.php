...
// Dimensiones, SMM 24/11/2022
$DimSeries = intval(ObtenerVariable("DimensionSeries"));
$SQL_Dimensiones = Seleccionar('uvw_Sap_tbl_Dimensiones', '*', "DimActive='Y'");

// Pruebas, SMM 24/11/2022
...
if (isset($_POST['P']) && ($_POST['P'] != "")) { //Grabar Entrada de compras
    ...
    try {
        if ($_POST['tl'] == 1) { //Actualizar
            ...
            $IdEvento = base64_decode($_POST['IdEvento']);
            $Type = 2;
            if (!PermitirFuncion(403)) { //Permiso para autorizar Entrada de compras
...
//            sqlsrv_close($conexion);
            //            if($_POST['tl']==0){//Creando Entrada
            //                header('Location:'.base64_decode($_POST['return']).'&a='.base64_encode("OK_ECompAdd"));
...
if (isset($_GET['dt_OV']) && ($_GET['dt_OV']) == 1) { //Verificar que viene de una Orden de compras
    $dt_OV = 1;
    ...
    if (!$SQL_CopiarOrdenToEntrada) {
		...
		$(document).ready(function() {
			swal({
				...
				text: 'No se pudo copiar la Orden en Entrada de compras.',
				type: 'error'
			});
		});
		</script>";
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if ($edit == 0) {echo "20";} else {echo "21";}?>&identradacompra=<?php if ($edit == 1) {echo base64_encode($row['ID_EntradaCompra']);} else {echo "0";}?>&evento=<?php if ($edit == 1) {echo base64_encode($row['IdEvento']);} else {echo "0";}?>&tipodoc=1&dim1='+dim1+'&dim2='+dim2+'&dim3='+dim3,'remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Advertencia!",
...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		$("#CardCode").change(function(){
			...
			<?php if ($dt_LS == 0 && $dt_OV == 0) { //Para que no recargue las listas cuando vienen de una llamada de servicio u Orden compra.?>
			$.ajax({
				type: "POST",
		...
		$("#Sucursal").change(function(){
			...
			$.ajax({
				type: "POST",
				url: "ajx_cbo_select.php?type=20&id="+Sucursal+"&serie="+Serie+"&tdoc=20",
				success: function(response){
			...
			if(Sucursal!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=17&type=1&name=OcrCode3&value="+Base64.encode(Sucursal)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=17&type=2&name=OcrCode3&value="+Base64.encode(Sucursal)+"&line=0&id=<?php echo $row['ID_EntradaCompra']; ?>&evento=<?php echo $IdEvento; ?>&actodos=1",
							success: function(response){
...
//			var frame=document.getElementById('DataGrid');
//			if(carcode!="" && almacen!=""){
//				frame.src="detalle_entrada_compra.php?id=0&type=1&usr=<?php //echo $_SESSION['CodUser'];?>&cardcode="+carcode+"&whscode="+almacen;
//			}else{
//				frame.src="detalle_entrada_compra.php";
//			}
//			$('.ibox-content').toggleClass('sk-loading',false);
			...
			if(document.getElementById('Dim1').value!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=17&type=1&name=OcrCode&value="+Base64.encode(document.getElementById('Dim1').value)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
								frame.src="detalle_entrada_compra.php?id=0&type=1&usr=<?php echo $_SESSION['CodUser']; ?>&cardcode="+document.getElementById('CardCode').value;
								$('.ibox-content').toggleClass('sk-loading',false);
							}
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=17&type=2&name=OcrCode&value="+Base64.encode(document.getElementById('Dim1').value)+"&line=0&id=<?php echo $row['ID_EntradaCompra']; ?>&evento=<?php echo $IdEvento; ?>&actodos=1",
							success: function(response){
								frame.src="detalle_entrada_compra.php?id=<?php echo base64_encode($row['ID_EntradaCompra']); ?>&evento=<?php echo base64_encode($IdEvento); ?>&type=2";
								$('.ibox-content').toggleClass('sk-loading',false);
							}
			...
			<div class="col-lg-12">
				...
				<?php include "includes/spinner.php";?>
					<div class="form-group">
						<div class="col-lg-6">
							<a href="sapdownload.php?id=<?php echo base64_encode('15'); ?>&type=<?php echo base64_encode('2'); ?>&DocKey=<?php echo base64_encode($row['DocEntry']); ?>&ObType=<?php echo base64_encode('20'); ?>&IdFrm=<?php echo base64_encode($row['IdSeries']); ?>" target="_blank" class="btn btn-outline btn-success"><i class="fa fa-download"></i> Descargar formato</a>
						...
						<div class="col-lg-6">
							<?php if ($row['DocDestinoDocEntry'] != "") {?>
								<a href="<?php if ($row['DocDestinoObjeto'] == 21) {echo "devolucion_compra";} else {echo "factura_compra";}?>.php?id=<?php echo base64_encode($row['DocDestinoDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocDestinoIdPortal']); ?>&tl=1" target="_blank" class="btn btn-outline btn-success pull-right m-l-sm">Ir a documento destino <i class="fa fa-mail-forward"></i></a>
							<?php }?>
							<?php if ($row['DocBaseDocEntry'] != "") {?>
								<a href="orden_compra.php?id=<?php echo base64_encode($row['DocBaseDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocBaseIdPortal']); ?>&tl=1" target="_blank" class="btn btn-outline btn-success pull-right m-l-sm"><i class="fa fa-mail-reply"></i> Ir a documento base</i></a>
							<?php }?>
							<button type="button" onClick="javascript:location.href='actividad.php?dt_DM=1&Cardcode=<?php echo base64_encode($row['CardCode']); ?>&Contacto=<?php echo base64_encode($row['CodigoContacto']); ?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']); ?>&Direccion=<?php echo base64_encode($row['DireccionDestino']); ?>&DM_type=<?php echo base64_encode('20'); ?>&DM=<?php echo base64_encode($row['DocEntry']); ?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('traslado_inventario.php'); ?>'" class="alkin btn btn-outline btn-primary pull-right"><i class="fa fa-plus-circle"></i> Agregar actividad</button>
						</div>
					</div>
				</div>
			</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-5">
							<select name="SucursalDestino" class="form-control select2" id="SucursalDestino" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {echo "disabled='disabled'";}?>>
							  ...
							  <?php if (($edit == 1) || ($sw_error == 1) || ($dt_LS == 1) || ($dt_OV == 1)) {while ($row_SucursalDestino = sqlsrv_fetch_array($SQL_SucursalDestino)) {?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="col-lg-8">
					...
					<div class="form-group">
						...
						<div class="col-lg-10">
							<textarea name="Comentarios" form="CrearEntradaCompra" rows="4" class="form-control" id="Comentarios" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {echo "readonly";}?>><?php if ($edit == 1 || $sw_error == 1) {echo $row['Comentarios'];}?></textarea>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="col-lg-3">
						<div class="btn-group pull-right">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="alkin dropdown-item" href="factura_compra.php?dt_EC=1&Cardcode=<?php echo base64_encode($row['CardCode']); ?>&Dim1=<?php echo base64_encode($row['OcrCode']); ?>&Dim2=<?php echo base64_encode($row['OcrCode2']); ?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']); ?>&Direccion=<?php echo base64_encode($row['DireccionDestino']); ?>&Almacen=<?php echo base64_encode($row['WhsCode']); ?>&Contacto=<?php echo base64_encode($row['CodigoContacto']); ?>&EC=<?php echo base64_encode($row['ID_EntradaCompra']); ?>&Evento=<?php echo base64_encode($row['IdEvento']); ?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>">Factura de compra</a></li>

								<li><a class="alkin dropdown-item" href="devolucion_compra.php?dt_EC=1&Cardcode=<?php echo base64_encode($row['CardCode']); ?>&CCosto=<?php echo base64_encode($row['OcrCode2']); ?>&Area=<?php echo base64_encode($row['OcrCode']); ?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']); ?>&Direccion=<?php echo base64_encode($row['DireccionDestino']); ?>&Almacen=<?php echo base64_encode($row['WhsCode']); ?>&Contacto=<?php echo base64_encode($row['CodigoContacto']); ?>&EC=<?php echo base64_encode($row['ID_EntradaCompra']); ?>&Evento=<?php echo base64_encode($row['IdEvento']); ?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>">Devolución de compra</a></li>
                            </ul>
                        </div>
					</div>
					<?php }?>
