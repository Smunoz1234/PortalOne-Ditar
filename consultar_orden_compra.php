...
$Cons = "Select * From uvw_Sap_tbl_OrdenesCompras Where $WhereFecha $Filtro Order by DocNum DESC";
...
if (isset($_GET['IDTicket']) && $_GET['IDTicket'] != "") {
    ...
    $Cons = "Select * From uvw_Sap_tbl_OrdenesCompras Where $Where";
}
...
<div id="wrapper">
    ...
    <div id="page-wrapper" class="gray-bg">
        ...
        <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-sm-8">
                    <h2>Consultar orden de compra</h2>
                    <ol class="breadcrumb">
                        <li>
         ...
         <div class="wrapper wrapper-content">
             <div class="row">
			    ...
			    <div class="ibox-content">
				  ...
				  <form action="consultar_orden_compra.php" method="get" id="formBuscar" class="form-horizontal">
...
if ($sw == 1) {
    while ($row = sqlsrv_fetch_array($SQL)) {?>
						 <tr class="gradeX">
							...
							<td><?php if ($row['ID_LlamadaServicio'] != 0) {?><a href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('consultar_orden_compra.php'); ?>&tl=1" target="_blank"><?php echo $row['DocNumLlamadaServicio']; ?></a><?php } else {echo "--";}?></td>
							<td><?php if ($row['DocBaseDocEntry'] != "") {?><a href="solicitud_compra.php?id=<?php echo base64_encode($row['DocBaseDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocBaseIdPortal']); ?>&tl=1" target="_blank"><?php echo $row['DocBaseDocNum']; ?></a><?php } else {echo "--";}?></td>
							<td><?php if ($row['DocDestinoDocEntry'] != "") {?><a href="entrada_compra.php?id=<?php echo base64_encode($row['DocDestinoDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocDestinoIdPortal']); ?>&tl=1" target="_blank"><?php echo $row['DocDestinoDocNum']; ?></a><?php } else {echo "--";}?></td>
							...
							<td><a href="orden_compra.php?id=<?php echo base64_encode($row['ID_OrdenCompra']); ?>&id_portal=<?php echo base64_encode($row['IdDocPortal']); ?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('consultar_orden_compra.php'); ?>" class="alkin btn btn-success btn-xs"><i class="fa fa-folder-open-o"></i> Abrir</a> <a href="sapdownload.php?id=<?php echo base64_encode('15'); ?>&type=<?php echo base64_encode('2'); ?>&DocKey=<?php echo base64_encode($row['ID_OrdenCompra']); ?>&ObType=<?php echo base64_encode('22'); ?>&IdFrm=<?php echo base64_encode($row['IdSeries']); ?>" target="_blank" class="btn btn-warning btn-xs"><i class="fa fa-download"></i> Descargar</a></td>
						</tr>
					<?php }
}?>
                    </tbody>
