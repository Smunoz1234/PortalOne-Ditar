...
if(isset($_POST['P'])&&($_POST['P']!="")){//Grabar Solicitud de compras
	...
	try{
		...
		if($SQL_CabeceraSolicitudCompra){
			...
			try{
//				$Parametros=array(
//					'pIdSolicitudCompra' => $IdSolicitudCompra,
//					'pIdEvento' => $IdEvento,
//					'pLogin'=>$_SESSION['User']
//				);
//				$Metodo="AppPortal_InsertarSolicitudCompra";
//				$Resultado=EnviarWebServiceSAP($Metodo,$Parametros);
				...
				if($Resultado->Success==0){
					...
					if($_POST['tl']==0){//Creando Solicitud	
						...
						$SQL_ConsID=Seleccionar('uvw_Sap_tbl_SolicitudCompra','ID_SolicitudCompra',"IdDocPortal='".$IdSolicitudCompra."'");
...
$SQL_UnidadNegocio=Seleccionar('uvw_Sap_tbl_DimensionesReparto','*','DimCode=3');
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "5";}else{echo "6";}?>&identregaventa=<?php if($edit==1){echo base64_encode($row['ID_SolicitudCompra']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			swal({
				title: "¡Advertencia!",
           ...
           <div class="col-lg-12">
              <form action="solicitud_compra.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearSolicitudCompra">
				...
				<div class="col-lg-4">
					...
					<div class="form-group">
						<label class="col-lg-5">Válido hasta</label>
						<div class="col-lg-7 input-group date">
				...
				<div class="form-group">
					<label class="col-lg-1 control-label">Operaciones</label>
					<div class="col-lg-3">
					...
					<div class="col-lg-3">
                    	<select name="Sucursal" class="form-control" id="Sucursal" required="required" <?php if(($edit==1)&&($row['Cod_Estado']=='C')){echo "disabled='disabled'";}?>>
                          ...
                          <?php if($edit==1){
									...
									<option value="<?php echo $row_Sucursal['IdSucursal'];?>" <?php if(($edit==1)&&(isset($row['OcrCode2']))&&(strcmp($row_Sucursal['IdSucursal'],$row['OcrCode2'])==0)){ echo "selected=\"selected\"";}?>><?php echo $row_Sucursal['DeSucursal'];?></option>
							<?php 	}
								}?>
						</select>
               	  	</div>
				...
				<div class="tabs-container">  
					...
					<div class="tab-content">
						<div id="tab-1" class="tab-pane active">
							<iframe id="DataGrid" name="DataGrid" style="border: 0;" width="100%" height="300" src="<?php if($edit==0){echo "detalle_solicitud_compra.php";}else{echo "detalle_solicitud_compra.php?id=".base64_encode($row['ID_SolicitudCompra'])."&evento=".base64_encode($row['IdEvento'])."&docentry=".base64_encode($row['DocEntry'])."&type=2&status=".base64_encode($row['Cod_Estado']);}?>"></iframe>
						</div>
						<?php if($edit==1){?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="col-lg-8">
					...
					<div class="form-group">
						...
						<div class="col-lg-10">
							<textarea name="Comentarios" form="CrearSolicitudCompra" rows="4" class="form-control" id="Comentarios" <?php if(($edit==1)&&($row['Cod_Estado']=='C')){echo "readonly";}?>><?php if($edit==1){echo $row['Comentarios'];}?></textarea>
						</div>
					</div>
				</div>
				<div class="col-lg-4">
				...
				<div class="form-group">
					<div class="col-lg-9">
						...
						<?php }?>
						<?php 
							$EliminaMsg=array("&a=".base64_encode("OK_EVenAdd"),"&a=".base64_encode("OK_EVenUpd"));//Eliminar mensajes
