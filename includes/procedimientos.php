...
if(isset($_GET['type'])&&$_GET['type']!=""){
	...
	elseif($_GET['type']==23){//Agregar item en las series de FE
		...
		$SQL=EjecutarSP('sp_tbl_FacturacionElectronica_Series',$Parametros,0,2);
		if($SQL){
			echo date('h:i:s a');
		}else{
			throw new Exception('Error al insertar el nuevo item');
	...
	elseif($_GET['type']==24){//Eliminar una linea en las series de FE
		$SQL_Del=Eliminar("tbl_FacturacionElectronica_Series","[ID]='".$_GET['linenum']."'",2);
		if($SQL_Del){
			echo "*Ok*";
		}
	}
	...
	elseif($_GET['type']==51){//Eliminar una linea del carrito en la Devolucion de compras
		if($_GET['edit']==1){
			$Cons="Delete From tbl_DevolucionCompraDetalleCarrito Where LineNum='".$_GET['linenum']."' And CardCode='".$_GET['cardcode']."' And Usuario='".$_SESSION['CodUser']."'";
			$SQL_Cons=sqlsrv_query($conexion,$Cons);
			if($SQL_Cons){
				echo "*Ok*";
			}
			...
			$LimpiarOrden=EjecutarSP('sp_tbl_DevolucionCompraDetalle_DelLine',$Parametros);
			if($LimpiarOrden){
				echo "*Ok*";
			}
		}
