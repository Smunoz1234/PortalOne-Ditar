...
function PermitirAcceso($Permiso){//Para evitar acceder a la pagina
	global $conexion;
	$PaginaError="404.php";
	$Consulta="Select 1 From uvw_tbl_PermisosPerfiles Where ID_Permiso='".$Permiso."' and ID_PerfilUsuario='".$_SESSION['Perfil']."'";
...
function PermitirFuncion($Permiso){//Para evitar acceder a una opcion en particular
	...
	if(is_array($Permiso)){
		foreach($Permiso as $Valor){
			$Consulta="Select 1 From uvw_tbl_PermisosPerfiles Where ID_Permiso='".$Valor."' and ID_PerfilUsuario='".$_SESSION['Perfil']."'";
			$SQL=sqlsrv_query($conexion,$Consulta,array(),array( "Scrollable" => 'Buffered' ));
			$Num=sqlsrv_num_rows($SQL);
			if($Num==1){
				$return = true;
...
function InsertarLog($Type, $Code, $Consulta){
	...
	$Cons="EXEC sp_tbl_Log '".$User."','".$Type."','".$Code."','".$Consulta."'";
	...
	if(!sqlsrv_query($conexion,$Cons)){
		$Cons="EXEC sp_tbl_Log '".$User."','".$Type."','".$Code."','".utf8_encode($Consulta)."'";
		if(!sqlsrv_query($conexion,$Cons)){
...
function FormatoFechaToSAP($Fecha,$Hora=''){//Dar formato a la fecha para insertar en BD
	$FechaResult="";	
...
function SubComent($pComents, $Len=100){//Substraer solo la cantidad de caracteres del string
	$result="";
	
	$result = (strlen($pComents)>$Len) ? substr($pComents,0,$Len)."..." : $pComents;
...
function SeleccionarGroupBy($pVista, $pCampos, $pWhere='', $pGroupBy='', $pOrderBy='', $pOrderType='', $pType=1, $pDebugMode=0){//Seleccionar datos de una tabla
	if($pType==1){//Consulta a SQL SERVER
		global $conexion;
...
function Eliminar($pVista, $pWhere='', $pIdReg=0, $pType=1, $pDebugMode=0){//Eliminar datos de una tabla
	if($pType==1){//Consulta a SQL SERVER
		global $conexion;
...
function ObtenerValorDefecto($TipoObjeto, $NombreCampo){//Obtener valor por defecto configurado en el usuario dependiendo del documento
	global $conexion;
...
function EliminarArchivo($pRuta){//Eliminar un archivo en una ruta
...
function CrearObtenerDirRuta($pRuta){//Crear y retornar la carpeta de la ruta que se pe pase
	$carp_anexos=$pRuta;
...
function FormatoNombreAnexo($pAnexo, $pAddPrefijo=true){//Cambia el nombre del anexo para guardarlo
...
function SumarTotalLotesEntregar($ItemCode, $LineNum, $WhsCode, $CardCode, $Objtype, $Sentido, $Usuario){//Sumar el total que se está entregando en los lotes de SAP B1
	global $conexion;
	$Parametros=array(
		"'".$ItemCode."'",
		"'".$LineNum."'",
	...
	$SQL=EjecutarSP('sp_ConsultarLotesTotalEntregar',$Parametros);
	$row=sqlsrv_fetch_array($SQL);
	return number_format($row['Total'],0);
}

function SumarTotalSerialesEntregar($ItemCode, $LineNum, $WhsCode, $CardCode, $Objtype, $Usuario){//Sumar el total que se está entregando en los lotes de SAP B1
	global $conexion;
	$Parametros=array(
		"'".$ItemCode."'",
		"'".$LineNum."'",
	...
	$SQL=EjecutarSP('sp_ConsultarSerialesTotalEntregar',$Parametros);
	$row=sqlsrv_fetch_array($SQL);
	return number_format($row['Total'],0);
}
...
function ObtenerHostURL(){//Obtiene solamente la ruta del servidor, sin el archivo .php
...
function ValidarEmail($str){//Validar que existe un email verificando los MX
...
function EnviarCorreo($pID, $pTipoObjeto, $pIdPlantilla){//Otra forma de enviar un correo, utilizando el SP de SQL
