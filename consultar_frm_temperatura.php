...
//Usuarios
$SQL_Usuarios=Seleccionar('uvw_tbl_MonitoreoTemperaturas','DISTINCT id_usuario_creacion, nombre_usuario_creacion','','nombre_usuario_creacion');
         ...
         <div class="wrapper wrapper-content">
             ...
             <div class="row">
			    ...
			    <div class="ibox-content">
				  ...
				  <form action="consultar_frm_temperatura.php" method="get" id="formBuscar" class="form-horizontal">
						...
						<div class="form-group">
					  	...
					  	<div class="form-group">
							<div class="col-lg-10">
								<a href="exportar_excel.php?exp=17&Cons=<?php echo base64_encode(implode(",",$Param));?>&sp=<?php echo base64_encode("sp_ConsultarFormTemp");?>">
			    ...
			    <div class="ibox-content">
					...
					<div class="table-responsive">
							...
							<?php while($row=sqlsrv_fetch_array($SQL)){ ?>
								<tr id="tr_Resum<?php echo $row['id_monitoreo_temperatura'];?>" class="trResum">
									<td><?php echo $row['id_monitoreo_temperatura'];?></td>
									<td><?php echo $row['socio_negocio'];?></td>
									<td><?php echo $row['id_direccion_destino'];?></td>
									<td><?php echo $row['bodega_puerto'];?></td>
									<td><?php echo number_format($row['temperatura_ambiente'],2);?></td>
									<td><?php echo number_format($row['temperatura_bodega'],2);?></td>
									<td><?php echo number_format($row['humedad_ambiente'],2);?></td>
									<td><?php echo number_format($row['humedad_bodega'],2);?></td>
									<td><?php if(strlen($row['observaciones'])>140){echo substr($row['observaciones'],0,140)."...";}else{echo $row['observaciones'];}?></td>
									<td id="comentCierre<?php echo $row['id_monitoreo_temperatura'];?>"><?php if(strlen($row['comentarios_cierre'])>140){echo substr($row['comentarios_cierre'],0,140)."...";}else{echo $row['comentarios_cierre'];}?></td>
									<td><?php echo $row['fecha_hora']->format('Y-m-d H:i');?></td>
									<td><?php echo $row['nombre_usuario_creacion'];?></td>	
									<td><span id="lblEstado<?php echo $row['id_monitoreo_temperatura'];?>" <?php if($row['estado']=='O'){echo "class='label label-info'";}elseif($row['estado']=='A'){echo "class='label label-danger'";}else{echo "class='label label-primary'";}?>><?php echo $row['nombre_estado'];?></span></td>
									<td class="text-center form-inline w-80">
										<?php if($row['estado']=='O'){?>
											<button id="btnEstado<?php echo $row['id_monitoreo_temperatura'];?>" class="btn btn-success btn-xs" onClick="CambiarEstado('<?php echo $row['id_monitoreo_temperatura'];?>');" title="Cambiar estado"><i class="fa fa-pencil"></i></button>
										<?php }?>
										
										<button id="btnDetalle<?php echo $row['id_monitoreo_temperatura'];?>" class="btn btn-primary btn-xs" onClick="VerDetalle('<?php echo $row['id_monitoreo_temperatura'];?>');" title="Ver detalle"><i class="fa fa-list"></i></button>
										
										<a href="filedownload.php?file=<?php echo base64_encode("MonitoreoTemperaturas/DescargarFormatos/".$row['id_monitoreo_temperatura']."/".$_SESSION['User']);?>&api=1" target="_blank" class="btn btn-warning btn-xs" title="Descargar"><i class="fa fa-download"></i></a>
									</td>
									<td class="text-center">
										<?php if($row['estado']=='O'){?>
										<div class="checkbox checkbox-success" id="dvChkSel<?php echo $row['id_monitoreo_temperatura'];?>">											
											<input type="checkbox" class="chkSelOT" id="chkSelOT<?php echo $row['id_monitoreo_temperatura'];?>" value="" onChange="SeleccionarOT('<?php echo $row['id_monitoreo_temperatura'];?>');" aria-label="Single checkbox One"><label></label>
										</div>
										<?php }?>
									</td>
								</tr>
...
function CambiarEstado(id,lote=false){
	...
	$.ajax({
		...
		data:{
			id:id,
			frm: 'MonitoreoTemperaturas',
			nomID: 'id_monitoreo_temperatura'
		},
		success: function(response){
			$('.ibox-content').toggleClass('sk-loading',false);
...
function VerDetalle(id){
	...
	$.ajax({
		type: "POST",
		url: "consultar_frm_temperatura_detalle.php",
		data:{
			id: id
		},
		success: function(response){
